...
from tokenizer.tokenizer_image.training.losses.lpips import LPIPS
from tokenizer.tokenizer_image.training.losses.discriminator_patchgan import NLayerDiscriminator as PatchGANDiscriminator
from tokenizer.tokenizer_image.training.losses.discriminator_stylegan import Discriminator as StyleGANDiscriminator
...
class VQLoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
                 ...
                 codebook_weight=1.0, perceptual_weight=1.0, entropy_weight=1.0, using_gan_GAN_loss=True,
                 use_clip_loss=False, pretrain_entropy=False
    ):
        super().__init__()
        # discriminator loss
        ...
        print("Using GAN loss?", using_gan_GAN_loss)
        self.entropy_weight = entropy_weight ##static weight to cal save loss
        self.using_GAN_loss = using_gan_GAN_loss
        self.gt_entropy_loss_weight = entropy_weight
        self.use_clip_loss = use_clip_loss
        self.pretrain_entropy = pretrain_entropy
        # discriminator
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, 
                logger=None, log_every=100, entropy_loss=None, bpp=None):
        # generator update
        # print("Entropy Weight: ", self.entropy_weight)
        ...
        if optimizer_idx == 0:
            ...
            #     adapt_even_weight = 0
            # else:
            #     adapt_even_weight = 0.2 * self.entropy_weight * entropy_loss.item()/abs(codebook_loss[2].item()+1e-8)

            if not self.pretrain_entropy:
                loss = self.rec_weight * rec_loss + \
                self.perceptual_weight * p_loss + \
                disc_adaptive_weight * disc_weight * generator_adv_loss + \
                codebook_loss[0] + codebook_loss[1] + self.gt_entropy_loss_weight * bpp
            else:
                loss = bpp

            # print(f"self.gt_entropy_loss_weight:{self.gt_entropy_loss_weight}, bpp:",bpp)
            # print(self.gt_entropy_loss_weight * codebook_loss[2])

            # loss = loss + 0.1*(codebook_loss[4]-codebook_loss[5])

            # print("Entropy weight:", self.entropy_weight)
            # print("gt_entropy_loss_weight:", self.gt_entropy_loss_weight)
            if bpp is not None:
                save_loss = self.rec_weight * rec_loss + self.perceptual_weight * p_loss + bpp*self.entropy_weight
            else:
                save_loss = self.rec_weight * rec_loss + self.perceptual_weight * p_loss
            ...
            if global_step % log_every == 0:
                ...
                logger.info(f"(Generator) rec_loss: {rec_loss:.4f}, perceptual_loss: {p_loss:.4f}, "+
                            f"vq_loss: {codebook_loss[0]:.4f}, commit_loss: {codebook_loss[1]:.4f}, Gt Entropy_loss: {codebook_loss[2]:.4f}, Gt sample entropy:{codebook_loss[4]:.4f}, Gt avg entropy:{codebook_loss[5]:.4f}, Gt_onehot_sample_entropy:{codebook_loss[6]:.4f}, Conditional_Entropy:{codebook_loss[7]:.4f}, "+
                            f"codebook_usage: {codebook_loss[3]:.4f}, generator_adv_loss: {generator_adv_loss:.4f}, "+
                            f"disc_adaptive_weight: {disc_adaptive_weight:.4f}, disc_weight: {disc_weight:.4f}" + f", bpp: {bpp}")
            loss_dict = {
                "rec_loss": rec_loss,
                "p_loss": p_loss,
                "generator_adv_loss": generator_adv_loss,
                "codebook_loss0": codebook_loss[0],
                "codebook_loss1": codebook_loss[1],
                "Gt_entropy_loss": codebook_loss[2],    
                "Gt_sample_entropy": codebook_loss[4],  
                "Gt_avg_entropy": codebook_loss[5],        
                "Gt_onehot_sample_entropy": codebook_loss[6],
                "Cd_entropy": codebook_loss[7],
                "save_loss": save_loss,
                "Entropy_loss":self.gt_entropy_loss_weight * bpp, 
                "bpp":bpp
            }
...
class ARLoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
                 ...
                 use_entropy_loss = True, using_gan_GAN_loss=True
    ):
        super().__init__()
        # discriminator loss
        assert disc_type in ["patchgan", "stylegan"]
    ...
    def forward(self, inputs, reconstructions, global_step, last_layer=None, optimizer_idx=0,
                logger=None, log_every=100, entropy_loss=None, bpp=0., testing=False):
        # generator update
        # print("Entropy Weight: ", self.entropy_weight)
        #print("Entropy loss:",entropy_loss)
        loss_dict = {}
        ...
        if optimizer_idx == 0:
            ...
            if reconstructions is not None:
                ...
                p_loss = self.perceptual_weight * p_loss
                generator_adv_loss = disc_adaptive_weight * disc_weight * generator_adv_loss
                if dist.get_rank() == 0 and global_step%log_every==0:
                    if testing:
                        logger.info(f"Testing: (Generator) rec_loss: {rec_loss:.4f}, perceptual_loss: {p_loss:.4f}, "+
                ...
                loss_dict = {
                    ...
                    "p_loss": p_loss,
                    "generator_adv_loss": generator_adv_loss,
                    "entropy_loss":entropy_loss if entropy_loss is not None else 0,
                }

            # loss = self.rec_weight * rec_loss + \
            #     self.perceptual_weight * p_loss + \
            #     disc_adaptive_weight * disc_weight * generator_adv_loss + \
            #     codebook_loss[0] + codebook_loss[1] + self.gt_entropy_loss_weight * codebook_loss[2]
            
                loss = loss + self.rec_weight * rec_loss + self.perceptual_weight * p_loss + disc_adaptive_weight * disc_weight * generator_adv_loss
                save_loss =  self.rec_weight * rec_loss + self.perceptual_weight * p_loss
